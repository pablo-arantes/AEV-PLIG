...
graphs_dict = {**pdbbind_graphs, **bindingnet_graphs, **local_graphs}
...
enriched = pd.read_csv("data/enriched.csv", index_col=0)
enriched['split'] = 'train'
enriched = enriched[['unique_id','pK','split']]

# combine pdbbind2020 and bindingnet index sets
data = pd.concat([pdbbind, bindingnet, enriched], ignore_index=True)
print(data[['split']].value_counts())


dataset = 'pdbbind_U_bindingnet_ligsim90_enriched'


df = data[data['split'] == 'train']
train_ids, train_y = list(df['unique_id']), list(df['affinity'])

df = data[data['split'] == 'valid']
valid_ids, valid_y = list(df['unique_id']), list(df['affinity'])

df = data[data['split'] == 'test']
test_ids, test_y = list(df['unique_id']), list(df['affinity'])
